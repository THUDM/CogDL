...
class Dataset(jittor.dataset.Dataset):
    ...
    def num_classes(self):
        ...
        if hasattr(self, "y") and self.y is not None:
            ...
            return 0
        return y.max().item() + 1 if len(y.shape) == 1 else y.size(1)
...
class MultiGraphDataset(Dataset):
    ...
    def num_classes(self):
        if hasattr(self, "y"):
            ...
            y = jittor.concat([x.y for x in self.data], dim=0)
        else:
            return 0
        return y.max().item() + 1 if BF.dim(y) == 1 else y.size(1)
        ...
        for key in self.data.__old_keys__():
            ...
            if key == "edge_index":
                ...
                if isinstance(item, jittor.Var):
                    s = list(repeat(slice(None), BF.dim(item)))
