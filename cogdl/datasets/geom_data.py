...
def read_geom_data(folder, dataset_name):
    ...
    for split in range(10):
        ...
        with np.load(graph_split_file_path) as splits_file:
            ...
            train_mask = BF.tensor(train_mask, dtype=BF.dtype_dict["bool"])
            val_mask = BF.tensor(val_mask, dtype=BF.dtype_dict["bool"])
            test_mask = BF.tensor(test_mask, dtype=BF.dtype_dict["bool"])
