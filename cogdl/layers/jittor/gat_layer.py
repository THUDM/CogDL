...
class GATLayer(Module):
    ...
    def __init__(
        ...
        self.W = jittor.rand(in_feats, out_feats * nhead)

        self.a_l = jittor.zeros((1, nhead, out_feats))
        self.a_r = jittor.zeros((1, nhead, out_feats))
    ...
    def reset_parameters(self):
        def reset(data):
            stdv = math.sqrt(6.0 / (data.size(-2) + data.size(-1)))
    ...
    def execute(self, graph, x):
        h = jittor.matmul(x, self.W).view(-1, self.nhead, self.out_features)
        h[jittor.isnan(h)] = 0.0
