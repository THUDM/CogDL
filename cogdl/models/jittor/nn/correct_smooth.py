...
def autoscale_post(x, lower, upper):
    return jittor.clamp(x, lower, upper)
...
def pre_residual_correlation(preds, labels, split_idx):
    ...
    nnode = preds.shape[0]
    err = jittor.zeros(nnode, nclass)
    err[split_idx] = nn.one_hot(labels[split_idx], nclass).float().squeeze(1) - preds[split_idx]
    return err
...
def pre_outcome_correlation(preds, labels, label_nid):
    ...
    y = preds.clone()
    if len(label_nid) > 0:
        y[label_nid] = nn.one_hot(labels[label_nid], c).float().squeeze(1)
    return y
...
class CorrectSmooth(BaseModel):
    ...
    def __call__(self, graph, x, train_only=True):
        ...
        if train_only:
            ...
            label_nid = jittor.concat((train_nid, valid_nid))
            residual_nid = train_nid
...
class CorrectSmoothMLP(BaseModel):
    ...
    def execute(self, graph):
        if self.cache_x is not None:
            ...
            x = jittor.concat([graph.x, _x], dim=1)
