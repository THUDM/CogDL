...
class GraphConvBlock(nn.Module):
    def __init__(self, in_feats, out_feats, dropout=0.0, residual=False, norm=None, activation=None):
        super(GraphConvBlock, self).__init__()

        self.activation = get_activation(activation)
        self.graph_conv = GraphConv(in_feats, out_feats, residual=residual, norm=norm, activation=activation)
        if dropout > 0:
            self.dropout = nn.Dropout(dropout)
        else:
