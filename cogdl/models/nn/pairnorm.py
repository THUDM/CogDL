...
class PairNorm(BaseModel):
    ...
    def __init__(
        ...
        if pn_model == "GCN":
            self.pn_model = GCN(
                num_features, hidden_layers, num_classes, dropout, norm_mode, norm_scale
                ).to(self.device)
        elif pn_model == "SGC":
            self.pn_model = SGC(
                num_features, hidden_layers, num_classes, dropout, nlayer, norm_mode, norm_scale
            ).to(self.device)
            ...
            self.pn_model = GAT(
                num_features, hidden_layers, num_classes, dropout, nhead, norm_mode
            ).to(self.device)
        else:
            print("Provide valid pn_model argument")


    def setup_data(self, x, edge_index):
        if self.no_fea_norm:
            norm = 1e-6 + x.sum(dim=1, keepdim=True)
            x = x / norm
            
        if self.missing_rate > 0:
            erasing_pool = torch.arange(x.shape[0])
    ...
    def forward(self, x, edge_index):
        if self.edge_attr is None:
            self.edge_attr = row_normalization(x.shape[0], edge_index).to(self.device)
            self.setup_data(x, edge_index)
        return self.pn_model(self.x, edge_index, self.edge_attr)
