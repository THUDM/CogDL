...
if not torch.cuda.is_available():
    ...
    try:
        ...
        sddmm = load(
            name='sddmm', sources=[os.path.join(path, "sddmm.cpp"), os.path.join(path, "sddmm_kernel.cu")], verbose=False
        )

        def csrspmm(rowptr, colind, colptr, rowind, x, csr_data):
            return SPMMFunction.apply(rowptr, colind, colptr, rowind, x, csr_data)
