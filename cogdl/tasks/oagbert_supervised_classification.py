...
class ClassificationModel(torch.nn.Module):
    ...
    def __init__(self, encoder, tokenizer, num_class, device, model_name='SciBERT', include_fields=['title'], max_seq_length=512, freeze=False):
    ...
    def build_input(self, sample, labels=None):
        text_input = [self.tokenizer.cls_token_id] + self._encode(sample.get('title', '') if 'title' in self.include_fields else '')
        if 'abstract' in self.include_fields and len(sample.get('abstracts', [])) > 0:
            text_input += [self.tokenizer.sep_token_id] + self._encode(''.join(sample.get('abstracts', [])) if 'abstract' in self.include_fields else '')
        venue_input = self._encode(sample.get('venue', '') if 'venue' in self.include_fields else '')
        aff_input = [self._encode(aff) for aff in sample.get('affiliations', [])] if 'aff' in self.include_fields else []
        author_input = [self._encode(author) for author in sample.get('authors', [])] if 'author' in self.include_fields else []
...
class supervised_classification(BaseTask):
    ...
    def load_model(self):
        ...
        return ClassificationModel(model, tokenizer, self.num_class, self.devices, 'OAG-BERT', self.include_fields, 512, self.freeze)
