...
def train_model(model, graph, dataset, test_mask, device, device_ids):
    ...
    trainer = Trainer(epochs=3, early_stopping=True, patience=1, cpu=device == "cpu", device_ids=device_ids)
    trainer.run(model_wrapper1, dataset_wrapper)
    model.load_state_dict(torch.load("./checkpoints/model.pt"), False)
    model.to(device)
    test_score = evaluate(model, graph, mask=test_mask, device=device)
    return test_score
...
def test_dice_modification_attack():
    ...
    n_mod_ratio = 0.01
    ratio_delete = 0.02
    attack = DICE(int(graph.to_scipy_csr()[test_mask.cpu()].getnnz() * n_mod_ratio), ratio_delete, device=device)
    graph_attack = attack.attack(graph)
    test_score = evaluate(model_sur, graph_attack, mask=test_mask, device=device)
    print("After attack, test score of surrogate model: {:.4f}".format(test_score))
    test_score = evaluate(model_target, graph_attack, mask=test_mask, device=device)
...
def test_rand_injection_attack():
    ...
    attack = RAND_Inject(n_inject_max=10, n_edge_max=20, feat_lim_min=-1, feat_lim_max=1, device=device)
    graph_attack = attack.attack(model=model_sur, graph=graph, adj_norm_func=GCNAdjNorm)
    print(graph_attack)
    test_score_sur = evaluate(model_sur, graph_attack, mask=test_mask, device=device)
    print("Test score after attack for surrogate model: {:.4f}.".format(test_score_sur))
...
def test_robustgcn_defense():
    ...
    model_target = RobustGCN(
        in_feats=graph.num_features, hidden_size=16, dropout=0.5, out_feats=graph.num_classes, num_layers=2
    )
    test_score = train_model(model_target, graph, dataset, test_mask, device, device_ids)
    print("Test score before attack for target model: {:.4f}.".format(test_score))
    apply_pgd_modification_attack(model_sur, model_target, graph, test_mask, device)
