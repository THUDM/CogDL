...
model = GCN(
    in_feats=graph.num_features, hidden_size=64, out_feats=graph.num_classes, num_layers=2, dropout=0.5, activation=None
)
print(model)
...
trainer = Trainer(epochs=200, early_stopping=True, patience=50, cpu=device == "cpu", device_ids=[0])
...
dataset_wrapper = dw_class(dataset)
trainer = Trainer(epochs=100, early_stopping=True, patience=50, cpu=device == "cpu", device_ids=[0])
...
print(graph_attack)
test_score = evaluate(model, graph_attack, mask=test_mask, device=device)
