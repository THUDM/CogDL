...
from cogdl.experiments_jt import experiment
...
class MyGraphClassificationDataset(GraphDataset):
    ...
    def process(self):
        graphs = []
        for i in range(200):
            edges = BF.randint(0, 1000, (2, 30))
            label = BF.randint(0, 7, (1,))
            graphs.append(Graph(edge_index=edges, y=label))
        BF.save(graphs, self.path)
        return graphs
