...
def test_dice_modification_attack(model_sur, model_target, graph, test_mask, device):
...
def test_fga_modification_attack(model_sur, model_target, graph, test_mask, device):
...
def test_flip_modification_attack(model_sur, model_target, graph, test_mask, device):
...
def test_rand_modification_attack(model_sur, model_target, graph, test_mask, device):
...
def test_nea_modification_attack(model_sur, model_target, graph, test_mask, device):
...
def test_stack_modification_attack(model_sur, model_target, graph, test_mask, device):
...
def test_pgd_modification_attack(model_sur, model_target, graph, test_mask, device):
...
def test_prbcd_modification_attack(model_sur, model_target, graph, test_mask):
...
def test_fgsm_injection_attack(model_sur, model_target, graph, test_mask, device):
...
def test_pgd_injection_attack(model_sur, model_target, graph, test_mask, device):
...
def test_rand_injection_attack(model_sur, model_target, graph, test_mask, device):
    print("RAND injection attack...")
    attack = RAND_Inject(n_inject_max=100,
                n_edge_max=200,
                feat_lim_min=-1,
                feat_lim_max=1,
                device=device)
...
def test_speit_injection_attack(model_sur, model_target, graph, test_mask, device):
...
def test_tdgia_injection_attack(model_sur, model_target, graph, test_mask, device):
...
def test_gcnsvd_defense(model_sur, graph, dataset, test_mask, device, device_ids):
    ...
    test_pgd_modification_attack(model_sur, model_target, graph, test_mask, device)


def test_gatguard_defense(model_sur, graph, dataset, test_mask, device, device_ids):
...
def test_gcnguard_defense(model_sur, graph, dataset, test_mask, device, device_ids):
    print("GATGuard defense model...")
...
def test_robustgcn_defense(model_sur, graph, dataset, test_mask, device, device_ids):
...
def test_adversarial_train(model_sur, graph, dataset, test_mask, device, device_ids):
    model_target = GCN(
        in_feats=graph.num_features,
        hidden_size=64,
        out_feats=graph.num_classes,
...
if __name__ == "__main__":
    ...
    score_target = train_model(model_sur, graph, dataset, test_mask, device, device_ids)
    print("Test score before attack for target model: {:.4f}.".format(score_target))
    test_dice_modification_attack(model_sur, model_target, graph, test_mask, device)
    test_fga_modification_attack(model_sur, model_target, graph, test_mask, device)
    test_flip_modification_attack(model_sur, model_target, graph, test_mask, device)
    test_rand_modification_attack(model_sur, model_target, graph, test_mask, device)
    test_nea_modification_attack(model_sur, model_target, graph, test_mask, device)
    test_stack_modification_attack(model_sur, model_target, graph, test_mask, device)
    test_pgd_modification_attack(model_sur, model_target, graph, test_mask, device)
    test_prbcd_modification_attack(model_sur, model_target, graph, test_mask)
    test_fgsm_injection_attack(model_sur, model_target, graph, test_mask, device)
    test_pgd_injection_attack(model_sur, model_target, graph, test_mask, device)
    test_rand_injection_attack(model_sur, model_target, graph, test_mask, device)
    test_speit_injection_attack(model_sur, model_target, graph, test_mask, device)
    test_tdgia_injection_attack(model_sur, model_target, graph, test_mask, device)
    test_gcnsvd_defense(model_sur, graph, dataset, test_mask, device, device_ids)
    test_gatguard_defense(model_sur, graph, dataset, test_mask, device, device_ids)
    test_gcnguard_defense(model_sur, graph, dataset, test_mask, device, device_ids)
    test_robustgcn_defense(model_sur, graph, dataset, test_mask, device, device_ids)
    test_adversarial_train(model_sur, graph, dataset, test_mask, device, device_ids)