...
def hypersearch_mlp_wisconsin_all(trial):
    return {
...
def hypersearch_mlp_wisconsin_classic(trial):
    return {
...
def hypersearch_mlp_wisconsin_best(trial):
    return {
        ...
        "norm": trial.suggest_categorical("norm", ['layernorm']),

        "lr": trial.suggest_categorical("lr", [0.1]),
        "weight_decay": trial.suggest_categorical("weight_decay", [1e-04]),
        "epochs": trial.suggest_categorical("epochs", [2000]),
        "patience": trial.suggest_categorical("patience", [200]),
    }
...
if __name__ == "__main__":
    ...
    if False:
        experiment(
            ...
            seed = list(range(1)),
            split = list(range(10)),
            search_space = hypersearch_mlp_wisconsin_best,
            n_trials = 1,
            project = 'hypersearch',
            devices = [-1],
            # trial_log_path = './logs/trial/hypersearch_wisconsin_mlp_classic_try100.log',
