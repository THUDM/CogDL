...
def hypersearch_mlp_arxiv_all(trial):
    return {
...
def hypersearch_mlp_arxiv_classic(trial):
    return {
...
def hypersearch_mlp_arxiv_best(trial):
    return {
        ...
        "dropout": trial.suggest_categorical("dropout", [0.4]),
        "norm": trial.suggest_categorical("norm", ['batchnorm']),

        "lr": trial.suggest_categorical("lr", [0.003]),
...
if __name__ == "__main__":
    ...
    if False:
        experiment(
            ...
            model = model,
            seed = list(range(1)),
            search_space = hypersearch_mlp_arxiv_best,
            n_trials = 1,
            project = 'hypersearch',
            devices = [-1],
            # trial_log_path = './logs/trial/hypersearch_arxiv_mlp_classic_try100.log',
