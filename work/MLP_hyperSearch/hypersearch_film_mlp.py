...
def hypersearch_mlp_film_all(trial):
    return {
...
def hypersearch_mlp_film_classic(trial):
    return {
...
def hypersearch_mlp_film_best(trial):
    return {
        ...
        "dropout": trial.suggest_categorical("dropout", [0.80]),
        "norm": trial.suggest_categorical("norm", ['batchnorm']),

        "lr": trial.suggest_categorical("lr", [0.03]),
        "weight_decay": trial.suggest_categorical("weight_decay", [1e-06]),
        "epochs": trial.suggest_categorical("epochs", [2000]),
        "patience": trial.suggest_categorical("patience", [200]),
    }
...
if __name__ == "__main__":
    ...
    if False:
        experiment(
            ...
            seed = list(range(1)),
            split = list(range(10)),
            search_space = hypersearch_mlp_film_best,
            n_trials = 1,
            project = 'hypersearch',
            devices = [-1],
            # trial_log_path = './logs/trial/hypersearch_film_mlp_classic_try100.log',
